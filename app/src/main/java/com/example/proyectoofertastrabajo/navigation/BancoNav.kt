...
import com.example.proyectoofertastrabajo.roles.CrearConvocatoria
import com.example.proyectoofertastrabajo.roles.VisualizarSolicitud
...
fun BancoNav(
    ...
    NavHost(
        ...
        composable(
            "page_content/{companyName}/{salary}/{position}/{educationRequired}/{vacancies}/{phone}/{convocatoriaName}/{jobDescription}/{city}/{adminUnit}/{contractType}/{publicationDate}/{endDate}",
            arguments = listOf(
                ...
                navArgument("vacancies") { type = NavType.IntType },
            ...
            PageContent(
                ...
                position = backStackEntry.arguments?.getString("position") ?: "",
                educationRequired = backStackEntry.arguments?.getString("educationRequired") ?: "",
                vacancies = backStackEntry.arguments?.getInt("vacancies") ?: 0,
        ...
        composable(NavScreen.CrearConvocatoria.name) {
            CrearConvocatoria(navController = navController,drawerState = drawerState, scope = scope)
        }
        composable(NavScreen.VisualPdf.name) {
            VisualizarSolicitud(navController = navController,drawerState = drawerState, scope = scope)
        }
    }
}